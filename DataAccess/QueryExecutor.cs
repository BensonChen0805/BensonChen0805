...
namespace WITDataAccess
{
    public class QueryExecutor
    {
        ...
        public IList<WorkItemEntity> GenerateWorkItemEntity(string alias, DateTime startTime, DateTime endTime)
        {
            ...
            foreach (var id in ids)
            {
                ...
                if (workItemEntity.WitType == WitType.ServiceTicket)
                {
                    ...
                    int reopenTimes = 0;
                    var awaiting_start = new DateTime();
                    ...
                    foreach (var update in workItemUpdates)
                    {
                            ...
                            && update.Fields.ContainsKey("System.State"))
                        {
                            ...
                            {
                                awaiting_start = Convert.ToDateTime(update.Fields["System.ChangedDate"].NewValue);
                            }
                                ...
                                && !newState.Equals("Awaiting Customer Feedback", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                {
                                    totalAwaitingTimeInMin += (Convert.ToDateTime(update.Fields["System.ChangedDate"].NewValue) - awaiting_start).TotalMinutes;
                                }
                            }
        ...
        {
            int num = Int32.Parse(ConfigurationManager.AppSettings.Get("num"));
            string all = "";
            for (int i = 1; i <= num; i++)
            {
                string single_alias = "alias_" + i.ToString();
                string temp = ConfigurationManager.AppSettings.Get(single_alias);
                all += $"[System.AssignedTo] = '{temp}' OR ";
            }
            all = all.Remove(all.Length - 3, 3);
             
            //string all = $"[System.AssignedTo] = '{ConfigurationManager.AppSettings.Get("alias_4")}' OR [System.AssignedTo] = '{ConfigurationManager.AppSettings.Get("alias_7")}'";

            var wiql = new Wiql();
            if (alias.Equals("all@microsoft.com"))
            {
                wiql = new Wiql()
                {
                    
                    Query = "Select [Id] From WorkItems WHERE [System.TeamProject] = 'devdiv' " +
                            $"AND [Microsoft.VSTS.Common.ClosedDate] >= '{startTime}' AND [Microsoft.VSTS.Common.ClosedDate] <= '{endTime}' " +
                            $"AND ({all})"
            ...
            else
            {
                wiql = new Wiql()
                {
                    Query = $"Select [Id] From WorkItems WHERE [System.AssignedTo] ='{alias}' AND [Microsoft.VSTS.Common.ClosedDate] >= '{startTime}' AND [Microsoft.VSTS.Common.ClosedDate] <= '{endTime}'"
